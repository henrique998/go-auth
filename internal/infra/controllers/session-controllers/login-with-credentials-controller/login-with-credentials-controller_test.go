...
func TestLoginWithCredentialsController(t *testing.T) {
	assert := assert.New(t)
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	usecase := usecasesmocks.NewMockLoginWithCredentialsUseCase(ctrl)
	sut := NewLoginWithCredentialsController(usecase)
	...
	t.Run("It should be able to login with credentials", func(t *testing.T) {
		...
		data := request.LoginWithCredentialsRequest{
		...
		usecase.EXPECT().Execute(data).Return("access-token", "refresh-token", nil)
