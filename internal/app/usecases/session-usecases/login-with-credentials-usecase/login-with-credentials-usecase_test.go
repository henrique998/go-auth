...
func TestLoginWithCredentialsUseCase(t *testing.T) {
	...
	usecase := NewLoginWithCredentialsUseCase(
		mockAccountsRepo,
		mockDevicesRepo,
		mockLARepo,
		mockEmailProvider,
	...
	t.Run("It should not be able to login with credentials if pass does not exists", func(t *testing.T) {
		...
		account := entities.NewAccount("jhon doe", req.Email, "", "", 23, "")
	...
	t.Run("It should not be able to login with credentials with wrong password", func(t *testing.T) {
		...
		account := entities.NewAccount("jhon doe", email, hashedPass, "", 23, "")
	...
	t.Run("It should not be able to login with credentials with unveried email", func(t *testing.T) {
		...
		lastLoginCountry := "br"
		lastLoginCity := "sp"
		updatedAt := time.Now().Add(-2 * time.Hour * 24 * 2)
		account := entities.NewExistingAccount(
			...
			&lastLoginCountry,
			&lastLoginCity,
			time.Now().Add(-1*time.Hour*24*30),
			&updatedAt,
		)
	...
	t.Run("It should be able to login with credentials sending notification email because diferent geolocation info", func(t *testing.T) {
		...
		accessTokenStr, _ := utils.GenerateJWTToken(account.GetID(), time.Now().Add(15*time.Minute), os.Getenv("JWT_SECRET"))
		refreshTokenStr, _ := utils.GenerateJWTToken(account.GetID(), time.Now().Add(1*time.Hour*24*30), os.Getenv("JWT_SECRET"))

		mockAccountsRepo.EXPECT().FindByEmail(req.Email).Return(account)
		mockATProvider.EXPECT().GenerateAuthTokens(account.GetID()).Return(accessTokenStr, refreshTokenStr, nil)
