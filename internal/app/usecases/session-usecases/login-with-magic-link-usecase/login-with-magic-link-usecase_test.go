...
func TestLoginWithMagicLinkUseCase(t *testing.T) {
	...
	sut := NewLoginWithMagicLinkUseCase(
		mockAccountsRepo,
		mockMLRepo,
		mockDevicesRepo,
		mockATProvider,
	...
	t.Run("It should not be able to login with magic link", func(t *testing.T) {
		...
		expiresAt := time.Now().Add(15 * time.Minute)
		account := entities.NewAccount("jhon doe", "jhondoe@gmail.com", "", "", 23, "")
		ml := entities.NewMagicLink(account.GetID(), code, expiresAt)
		accessTokenStr, _ := utils.GenerateJWTToken(account.GetID(), expiresAt, os.Getenv("JWT_SECRET"))
		...
		mockMLRepo.EXPECT().Delete(ml.GetID()).Return(nil)
