...
func TestRequestMagicLink(t *testing.T) {
	...
	sut := NewRequestMagicLinkUseCase(
		mockAccountsRepo,
		mockMLProvider,
		mockEmailProvider,
	)
	...
	t.Run("It should be able to send request for a magic link", func(t *testing.T) {
		...
		mockEmailProvider.EXPECT().SendMail(account.GetEmail(), "Auth with magic link!", gomock.Any()).Return(nil)

		err := sut.Execute(account.GetEmail())
